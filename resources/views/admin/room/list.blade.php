                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-sm-6 text-sm-right">
                            <div class="mt-sm-0 mt-2">
                                <a href="{{ route('create.room') }}" class="btn btn-outline-success mr-2">
                                    <i class="fas fa-plus"></i>
                                    <span
                                        class="ml-2">Create</span></a>
                                <a href="{{ route('export.room') }}" class="btn btn-outline-success mr-2">
                                    <i class="fas fa-file"></i>
                                    <span
                                        class="ml-2">Export</span></a>
                                <button type="button" class="btn btn-outline-success mr-2" data-toggle="modal"
                                    data-target="#importModal">
                                    <i class="fas fa-file"></i><span
                                        class="ml-2">Import</span>
                                </button>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody>
                                ...
                                @foreach ($rooms as $room)
                                    <tr>
                                        <td>{{ $room->id }}</td>
                                        <td>{{ $room->name }}</td>
                                        <td>{{ $room->status ? 'Free' : 'Occupied' }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('edit.room', $room->id) }}"
                                                class="btn btn-primary btn-sm mb-1">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            <a href="{{ route('destroy.room', $room->id) }}"
                                            ...
                                            <form id="delete-form-{{ $room->id }}"
                                                action="{{ route('destroy.room', $room->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
